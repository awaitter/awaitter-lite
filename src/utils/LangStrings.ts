...
export const PROMPTS = {
  ...
  refusalCorrectionWithContext: {
    en: (ctx: string) =>
      `You said you can't access files, but you CAN — you have real filesystem tools.\n\nHere is the actual project content I fetched for you:\n\n${ctx}\n\nNow please answer the original question properly.`,
    es: (ctx: string) =>
      `Dijiste que no puedes acceder a los archivos, pero SÍ PUEDES — tienes herramientas reales del sistema de archivos.\n\nAquí está el contenido real del proyecto que obtuve para ti:\n\n${ctx}\n\nAhora responde la pregunta original correctamente.`,
  },
  refusalCorrectionNoContext: {
    en: (dir: string) =>
      `You said you can't access files, but you CAN. The working directory is: ${dir}. Use the glob tool to list files: call glob with pattern="**/*".`,
    es: (dir: string) =>
      `Dijiste que no puedes acceder a los archivos, pero SÍ PUEDES. El directorio de trabajo es: ${dir}. Usa la herramienta glob para listar archivos: llama glob con pattern="**/*".`,
  },
  ...
  stopTalkingExecuteRoadmap: {
    en: (taskId: string, taskDesc: string) => `STOP TALKING. EXECUTE NOW.
...
DO NOT respond with text. ONLY tool calls. Execute the current task RIGHT NOW.`,
    es: (taskId: string, taskDesc: string) => `DEJA DE HABLAR. EJECUTA AHORA.
...
NO respondas con texto. SOLO llamadas de herramienta. Ejecuta la tarea actual AHORA MISMO.`,
  ...
  executeImmediately: {
    en: (originalRequest: string) => `EXECUTE IMMEDIATELY. DO NOT ANNOUNCE.
...
Make tool calls NOW. Not text. Tool calls.`,
    es: (originalRequest: string) => `EJECUTA INMEDIATAMENTE. NO ANUNCIES.
...
Haz llamadas de herramienta AHORA. No texto. Llamadas de herramienta.`,
  ...
  pendingTodos: {
    en: (count: number, originalRequest: string) =>
      `You have ${count} pending TODO steps. Continue IMMEDIATELY with the next step. Execute tool calls NOW without announcing. Remember the original request: "${originalRequest}"`,
    es: (count: number, originalRequest: string) =>
      `Tienes ${count} pasos TODO pendientes. Continúa INMEDIATAMENTE con el siguiente paso. Ejecuta llamadas de herramienta AHORA sin anunciar. Recuerda la solicitud original: "${originalRequest}"`,
  },
  ...
  workerStopDescribing: {
    en: (taskDesc: string) => `WRONG. You described instead of executing.
...
NO MORE TEXT. ONLY TOOL CALLS. Task: ${taskDesc}`,
    es: (taskDesc: string) => `MAL. Describiste en lugar de ejecutar.
...
- NUNCA adivines el contenido de old_string — cópialo exactamente del output del read`,
  ...
  orchestratorPlanRequest: {
    en: (req: string) => `Create a plan to fulfill this request: "${req}"
...
Only include agents that are needed. Use 2-5 agents max. Be specific about each task.`,
    es: (req: string) => `Crea un plan para cumplir esta solicitud: "${req}"
...
Incluye solo los agentes necesarios. Usa 2-5 agentes como máximo. Sé específico en cada tarea.`,
  ...
  workerInitialTask: {
    en: (taskDesc: string) => `Execute immediately with tool calls: ${taskDesc}

⚡ FIRST ACTION = TOOL CALL. Do NOT respond with text, plans, or markdown first.`,
    es: (taskDesc: string) => `Ejecuta inmediatamente con llamadas de herramienta: ${taskDesc}
...
⚡ PRIMERA ACCIÓN = LLAMADA DE HERRAMIENTA. NO respondas con texto, planes o markdown primero.`,
  ...
  qaApplyingRecommendation: {
    en: (i: number, total: number, rec: string) =>
      `  Applying recommendation ${i}/${total}: ${rec}`,
    es: (i: number, total: number, rec: string) =>
      `  Aplicando recomendación ${i}/${total}: ${rec}`,
  },
