...
export class BashTool implements Tool {
   ...
   */
  private static readonly ALWAYS_BLOCKED: Array<{ re: RegExp; reason: string }> = [
   ...
   */
  static readonly CONFIRM_REQUIRED: Array<{ re: RegExp; reason: string; severity: 'high' | 'critical' }> = [
   ...
   */
  static getDangerInfo(command: string): { reason: string; severity: 'high' | 'critical' } | null {
    for (const { re, reason, severity } of BashTool.CONFIRM_REQUIRED) {
      if (re.test(command)) return { reason, severity };
    }
    return null;
  }
