...
exports.parseToolCallsFromContent = parseToolCallsFromContent;
/**
 ...
 */
function extractJsonObject(content, startIdx) {
 ...
 */
function tryParseToolCall(raw, id) {
 ...
 */
function extractFromJsonBlocks(content) {
    const calls = [];
    const positions = []; // [start, end] of each matched block
    let id = 0;
    let searchFrom = 0;
    while (true) {
 ...
 */
function extractFromBareJson(content) {
    const calls = [];
    const positions = [];
    let id = 0;
    let i = 0;
 ...
 */
function removeRegions(content, positions) {
    let result = content;
...
//# sourceMappingURL=ToolCallParser.js.map