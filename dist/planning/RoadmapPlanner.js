...
class RoadmapPlanner {
    ...
    static requiresRoadmap(userRequest) {
        ...
        const hasMultipleRequirements = ((requestLower.match(/\band\b/g) || []).length >= 2 ||
     ...
     */
    static detectProjectType(userRequest) {
        const requestLower = userRequest.toLowerCase();
     ...
     */
    static extractProjectName(userRequest) {
     ...
     */
    static calculateProgress(roadmap) {
     ...
     */
    static getCurrentSprint(roadmap) {
     ...
     */
    static startTask(roadmap, taskId) {
        for (const sprint of roadmap.sprints) {
     ...
     */
    static completeTask(roadmap, taskId, filesCreated, filesModified) {
        for (const sprint of roadmap.sprints) {
            const task = sprint.tasks.find(t => t.id === taskId);
            if (task) {
                ...
                const allTasksComplete = sprint.tasks.every(t => t.status === 'completed' || t.status === 'skipped');
                if (allTasksComplete) {
                    sprint.status = 'completed';
                    sprint.completedAt = new Date();
                }
                // Check if entire roadmap is completed
                const allSprintsComplete = roadmap.sprints.every(s => s.status === 'completed');
     ...
     * Mark task as failed
     */
    static failTask(roadmap, taskId, error) {
        for (const sprint of roadmap.sprints) {
            const task = sprint.tasks.find(t => t.id === taskId);
            if (task) {
                task.status = 'failed';
     ...
     * Skip a task
     */
    static skipTask(roadmap, taskId, reason) {
        for (const sprint of roadmap.sprints) {
            const task = sprint.tasks.find(t => t.id === taskId);
            if (task) {
                task.status = 'skipped';
...
exports.RoadmapPlanner = RoadmapPlanner;
//# sourceMappingURL=RoadmapPlanner.js.map