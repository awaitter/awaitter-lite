...
exports.getCharlBinPath = getCharlBinPath;
exports.isCharlProject = isCharlProject;
exports.getCharlKnowledgePack = getCharlKnowledgePack;
...
function findCharlRepo() {
    return CHARL_REPO_CANDIDATES.find(p => fs.existsSync(path.join(p, 'docs', 'SPECIFICATION.md'))) ?? null;
}
// ── Binary detection ─────────────────────────────────────────────────────────
function getCharlBinPath() {
    const candidates = [
 ...
 */
function isCharlProject(workingDir) {
    try {
 ...
 */
function getCharlKnowledgePack(compact = true) {
    const repo = findCharlRepo();
...
// ── Pack builder ─────────────────────────────────────────────────────────────
function buildKnowledgePack(runCmd, specContent, examples, compact) {
    const sections = [];
...
//# sourceMappingURL=CharlKnowledge.js.map